...
class Model(object):
  ...
  def build_inference_for_training(self):
    ...
    with tf.name_scope('egomotion_prediction'):
      self.egomotion, _ = nets.egomotion_net(self.image_stack, in_or_out=True,
    ...
    with tf.variable_scope('depth_prediction'):
      ...
      for i in range(self.seq_length):
        ...
        multiscale_disps_i, _ = nets.disp_net(image, in_or_out=True)
  ...
  def build_depth_test_graph(self):
    ...
    with tf.name_scope('depth_prediction'):
      with tf.variable_scope('depth_prediction'):
        ...
        # in_or_out to False.
        est_disp, _ = nets.disp_net(input_float, in_or_out=True)
  ...
  def build_egomotion_test_graph(self):
    ...
    with tf.name_scope('egomotion_prediction'):
        # TODO(rezama): Retrain published model with batchnorm params and set
        # in_or_out to False.
      egomotion, _ = nets.egomotion_net(image_stack, in_or_out=True,
                                        legacy_mode=self.legacy_mode)
